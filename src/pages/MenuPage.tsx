  ...
  return (
    <div className="pt-16 min-h-screen bg-gray-50">
      ...
      <div className="relative">
        <div className="h-64 bg-gradient-to-r from-amber-400 to-orange-500 relative overflow-hidden">
          <div 
          ...
          <div className="container mx-auto px-6 h-full flex items-end pb-6 relative z-10">
            ...
            {[...Array(4)].map((_, index) => (
              <div key={index} className="bg-white rounded-lg overflow-hidden shadow-md animate-pulse">
                <div className="flex flex-col md:flex-row">
                  <div className="h-36 md:w-1/3 bg-gray-200"></div>
                  <div className="p-4 flex-grow">
                    ...
                    <div className="h-6 bg-gray-200 rounded w-24 mt-auto ml-auto"></div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <AnimatePresence>
        {showCartPreview && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 left-6 md:left-auto md:right-6 md:w-80 bg-white rounded-lg shadow-xl p-4 z-50"
          >
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-bold text-gray-800">Mon Panier</h3>
              <span className="bg-amber-100 text-amber-700 px-2 py-1 rounded-full text-sm font-medium">{cartCount} articles</span>
            </div>
            <p className="text-gray-600 text-sm mb-4">Article ajouté au panier</p>
            <motion.button
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              className="w-full py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-lg font-medium shadow-md hover:shadow-lg transition-shadow"
            >
              Voir mon panier
            </motion.button>
          </motion.div>
      ...
      {cartCount > 0 && !showCartPreview && (
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="fixed bottom-6 right-6 bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-full w-14 h-14 flex items-center justify-center shadow-lg hover:shadow-xl transition-shadow z-40"
